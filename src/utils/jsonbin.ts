...
export const createBin = async (data, binName = 'bracket') => {
    if (!API_KEY) {
        console.error('JSONBin API key not found. Make sure VITE_JSONBIN_API_KEY is set in your .env file.');
        return null;
    }
...
export const getOfficialResults = async () => {
    ...
    if (!resultsBinId) {
        console.error('Results Bin ID not found. Make sure VITE_JSONBIN_RESULTS_BIN_ID is set in your .env file.');
        return null;
    }
...
const getMasterIndexBinId = () => {
    const id = import.meta.env.VITE_JSONBIN_MASTER_INDEX_BIN_ID;
    if (!id) {
        console.error('Master Index Bin ID not found. Make sure VITE_JSONBIN_MASTER_INDEX_BIN_ID is set in your .env file.');
    }
    return id;
};

// Fetches the entire master index content
export const getMasterIndex = async () => {
    ...
    if (response.status === 404) { // Handle case where index bin might be new/empty
...
export const updateMasterIndex = async (indexData) => {
    const indexBinId = getMasterIndexBinId();
    if (!indexBinId || !API_KEY) return null;
